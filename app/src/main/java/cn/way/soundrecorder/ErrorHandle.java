...
import cn.way.soundrecorder.fragment.ErrorDialogFragment;
 ...
 *     author: Way Lin
 *     date  : 2018.04.15
 *     desc  : using to show error information,
...
public class ErrorHandle {
     ...
     *
     * @param activity  the activity which should show error information, using to
     *                  getFragmentManager
     * @param errorCode the code of occur error, defined in ErrorHandle
     */
    public static void showErrorInfo(AppCompatActivity activity, int errorCode) {
        if (null == activity) {
            return;
        }
        LogUtil.e(TAG, "<showErrorInfo> errorCode = " + errorCode);
        switch (errorCode) {
            ...
            case ERROR_SD_UNMOUNTED_WHEN_IDLE:
                ...
                break;
            default:
                LogUtil.e(TAG, "<showErrorInfo> error code is out of range");
                break;
        }
    }
    ...
    public static void showErrorInfoInToast(Context context, int errorCode) {
        ...
        LogUtil.e(TAG, "<showErrorInfoInToast> errorCode = " + errorCode);
        switch (errorCode) {
            case ERROR_ACCESSING_DB_FAILED_WHEN_QUERY:
            ...
            case ERROR_SD_UNMOUNTED_WHEN_IDLE:
                ...
                break;
            default:
                LogUtil.e(TAG, "<showErrorInfoInToast> error code is out of range");
                break;
        }
    }
     ...
     * message.
     *
     * @param activity  the activity which should show DialogFragment, use to
     *                  getFragmentManager
     * @param titleID   the string resource id of title
     * @param messageID the string resource id of message
     */
    private static void showDialogFragment(AppCompatActivity activity, int titleID, int messageID) {
     ...
     *
     * @param context       the context which should show toast
     * @param errorStringId the string resource id of error info
     */
    private static void showToast(Context context, int errorStringId) {
        SoundRecorderUtils.getToast(context, errorStringId);
    }

    private static void removeOldErrorDialog(AppCompatActivity activity) {
        if (null == activity) {
            return;
        }
        FragmentManager fragmentManager = activity.getSupportFragmentManager();
        ...
        LogUtil.i(TAG, "<removeOldErrorDialog> oldFragment = " + oldFragment);
        if (null != oldFragment) {
            oldFragment.dismissAllowingStateLoss();
            LogUtil.i(TAG, "<removeOldErrorDialog> remove oldFragment");
        }
    }
}
