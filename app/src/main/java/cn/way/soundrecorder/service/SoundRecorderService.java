package cn.way.soundrecorder.service;
...
import cn.way.soundrecorder.Player;
...
import cn.way.soundrecorder.util.Util;

/**
 * <pre>
 *     author: Way Lin
 *     date  : 2018.04.15
 *     desc  : Sound Recorder Service
 * </pre>
 */

public class SoundRecorderService extends Service implements Player.PlayerListener,
        Recorder.RecorderListener, MediaScannerConnection.MediaScannerConnectionClient {
    ...
    /* Add for update time view through
    ...
    private final Runnable mUpdateTimer = new Runnable() {
        public void run() {
            LogUtil.i(TAG, "run()-mUpdateTimer running");
            if (STATE_RECORDING == mCurrentState) {
                ...
                LogUtil.i(TAG, "run()-mRemainingTime is:" + mRemainingTime);
            }
            if ((mRemainingTime <= 0) && (STATE_RECORDING == mCurrentState)) {
                LogUtil.i(TAG, "run()-stopRecordingAsync case1");
                stopRecordingAsync();
            } else if (mCurrentState == STATE_IDLE) {
                LogUtil.i(TAG, "run()-stopRecordingAsync case2");
                stopRecordingAsync();
            } else {
                if (null != mOnUpdateTimeViewListener) {
                    ...
                    try {
                        ...
                        LogUtil.i(TAG, "run()-IllegalStateException");
                        return;
                    }
                }
    ...
    private void displayToast(int code) {
        ...
        /** If error displayed, then set current state as STATE_IDLE. **/
        setState(STATE_IDLE);
    }
    ...
    /**
     * after add record to Media database,
     * use MediaScanner to scan file which has been added
     */
    @Override
    public void onMediaScannerConnected() {
        LogUtil.i(TAG, "<onMediaScannerConnected> scan file: " + mFilePathToScan);
        mConnection.scanFile(mFilePathToScan, null);
    }

    /**
     * after scan completed, update record item
     *
     * @param path
     * @param uri
     */
    ...
    public void onScanCompleted(String path, Uri uri) {
        LogUtil.i(TAG, "<onScanCompleted> start, path = " + path);
        ...
        LogUtil.i(TAG, "<onScanCompleted> update result = " + result);
        mConnection.disconnect();
        LogUtil.i(TAG, "<onScanCompleted> end");
    }

    @Override
    public void onError(Player player, int errorCode) {
        LogUtil.i(TAG, "<Player onError> errorCode = " + errorCode);
        reset();
    ...
    public void onStateChanged(Player player, int stateCode) {
        ...
        setState(stateCode);
        LogUtil.i(TAG, "onStateChanged(Player,int) preState = " + preState + ", mCurrentState = "
                + mCurrentState);
        if (mCurrentState == STATE_IDLE) {
            if ((STATE_PLAYING == preState) || (STATE_PAUSE_PLAYING == preState)) {
                LogUtil.i(TAG, "onStateChanged(Player,int) removeCallbacks mUpdateTimer.");
                mHandler.removeCallbacks(mUpdateTimer);
                abandonAudioFocus();
            }
            if (mRunForeground) {
            ...
            if (mCurrentState == STATE_PLAYING) {
                LogUtil.i(TAG, "onStateChanged(Player,int) post mUpdateTimer.");
            ...
            if (mRunForeground) {
                LogUtil.i(TAG, "onStateChanged(Player,int) update notificaton");
                showNotification(SoundRecorderService.this);
            } else {
                LogUtil.i(TAG, "onStateChanged(Player,int) show notificaton");
                showNotification(SoundRecorderService.this);
            }
        }
    }
    ...
    public void onStateChanged(Recorder recorder, int stateCode) {
        ...
        int preState = mCurrentState;
        setState(stateCode);
        LogUtil.i(TAG, "onStateChanged(Recorder,int) preState = " + preState + ", mCurrentState = "
                + mCurrentState);
        if (STATE_IDLE == mCurrentState) {
            ...
            if (STATE_RECORDING == mCurrentState) {
                ...
                mRemainingTime = mRemainingTimeCalculator.timeRemaining(false, false);
                mHandler.post(mUpdateTimer);
                LogUtil.i(TAG, "onStateChanged(Recorder,int) post mUpdateTimer.");
            }

            if (mRunForeground) {
                LogUtil.i(TAG, "onStateChanged(Recorder,int) update notificaton");
                showNotification(SoundRecorderService.this);
            } else {
                LogUtil.i(TAG, "onStateChanged(Recorder,int) show notificaton");
                showNotification(SoundRecorderService.this);
            }
        }
    }
    ...
    public void onError(Recorder recorder, int errorCode) {
        LogUtil.i(TAG, "<Recorder onError> errorCode = " + errorCode);
        reset();
        mHandler.removeCallbacks(mUpdateTimer);
        mOnErrorListener.onError(errorCode);
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (action.equals(ACTION_GOON_RECORD) && mCurrentState == STATE_PAUSE_RECORDING) {
            LogUtil.i(TAG, "<onStartCommand> ACTION_GOON_RECORD");
            startRecordingAsync(mCurrentRecordParams, 0, null);
        } else if (action.equals(ACTION_GOON_PLAY) && mCurrentState == STATE_PAUSE_PLAYING) {
            LogUtil.i(TAG, "<onStartCommand> ACTION_GOON_PLAY");
            goonPlaybackAsync();
        } else if (action.equals(ACTION_PAUSE)) {
            LogUtil.i(TAG, "<onStartCommand> ACTION_PAUSE");
            ...
            LogUtil.i(TAG, "<onStartCommand> ACTION_STOP");
    ...
    /**
     * Set all listener as service itself.
     */
    public void setAllListenerSelf() {
        ...
        LogUtil.i(TAG, "<setAllListenerSelf> set new mOnErrorListener");
    ...
    public void onCreate() {
        ...
        mFocusChangeListener = new AudioManager.OnAudioFocusChangeListener() {
            ...
            public void onAudioFocusChange(int focusChange) {
                LogUtil.i(TAG, "<onAudioFocusChange> audio focus changed to " + focusChange);
                if (focusChange == AudioManager.AUDIOFOCUS_GAIN) {
                    LogUtil.i(TAG, "<onAudioFocusChange> audio focus changed to AUDIOFOCUS_GAIN");
                    mGetFocus = true;
                } else if (focusChange == AudioManager.AUDIOFOCUS_LOSS
                        || focusChange == AudioManager.AUDIOFOCUS_LOSS_TRANSIENT) {
                    LogUtil.i(TAG, "<onAudioFocusChange> audio focus loss, stop recording");
                    mGetFocus = false;
                    if (mCurrentState == STATE_RECORDING ||
        ...
        /**
         * To make sure the toast is displayed in UI thread. Handler construct in the UI thread,
         * then uses the main looper for the mToastHandler. @{
         **/
        mToastHandler = new Handler() {
            public void handleMessage(android.os.Message msg) {
                LogUtil.i(TAG, "<mErrorHandler handleMessage>");
                switch (msg.what) {
                    case NOT_AVILABLE:
     ...
     */
    public boolean record(RecordParamsSetting.RecordParams params, int fileSizeLimit) {
        if (STATE_RECORDING == mCurrentState) {
            LogUtil.i(TAG, "<record> still in STATE_RECORDING, do nothing");
            return true;
        } else if (STATE_PAUSE_RECORDING == mCurrentState) {
            LogUtil.i(TAG, "<record> in STATE_PAUSE_RECORDING, mRecorder.goonRecording()");
            ...
            if ((STATE_PAUSE_PLAYING == mCurrentState) || (STATE_PLAYING == mCurrentState)) {
                LogUtil.i(TAG, "<record> in pause playing or playing, stopPlay first");
                stopPlay();
            }
            if (isCurrentFileEndWithTmp()) {
                LogUtil.i(TAG, "<record> delete not saved file: " + mCurrentFilePath);
                stopWatching();
                File file = new File(mCurrentFilePath);
                file.delete();
            }
            if (!isStorageMounted()) {
                LogUtil.i(TAG, "<record> no storage mounted");
                mOnErrorListener.onError(ErrorHandle.ERROR_NO_SD);
                reset();
                setState(STATE_IDLE);
                return false;
            } else if (isStorageFull(params)) {
                LogUtil.i(TAG, "<record> storage is full");
                ...
                LogUtil.i(TAG, "<record> start record");
                boolean res = false;
                if (requestAudioFocus()) {
                    res = mRecorder.startRecording(getApplicationContext(), params, fileSizeLimit);
                    LogUtil.i(TAG, "<record> mRecorder.startRecording res = " + res);
                    mCurrentRecordParams = params;
                    if (res && fileSizeLimit > 0) {
                        mRemainingTimeCalculator.setFileSizeLimit(mRecorder.getSampleFile(),
                                fileSizeLimit);
                    } else if (!res) {
                        abandonAudioFocus();
                    }
    ...
    public boolean pauseRecord() {
        ...
        if (STATE_PAUSE_RECORDING == mCurrentState) {
            LogUtil.i(TAG, "<pauseRecord> still in STATE_PAUSE_RECORDING, do nothing, return");
    ...
    public boolean stopRecord() {
        if ((STATE_PAUSE_RECORDING != mCurrentState) && (STATE_RECORDING != mCurrentState)) {
            LogUtil.i(TAG, "<stopRecord> not in pause record or record state, return");
            mOnErrorListener.onError(STATE_ERROR_CODE);
            return false;
        }
        abandonAudioFocus();
    ...
    public boolean saveRecord() {
        if ((null == mCurrentFilePath) || !mCurrentFilePath.endsWith(Recorder.SAMPLE_SUFFIX)) {
            LogUtil.i(TAG, "<saveRecord> no file need to be saved");
            mOnErrorListener.onError(STATE_ERROR_CODE);
            return false;
        }
        if (!isStorageMounted()) {
            LogUtil.i(TAG, "<saveRecord> no storage mounted");
            mOnErrorListener.onError(ErrorHandle.ERROR_NO_SD);
            return false;
        }
        mRecordSaving = true;
        ...
        if (null != currentFilePath) {
            ...
            LogUtil.d(TAG, "currentFilePath is null...");
            reset();
    ...
    public boolean discardRecord() {
        if (!isStorageMounted()) {
            LogUtil.i(TAG, "<discardRecord> no storage mounted");
            mOnErrorListener.onError(ErrorHandle.ERROR_NO_SD);
            return false;
        }
        if (null == mCurrentFilePath) {
            LogUtil.i(TAG, "<discardRecord> file path is null.");
            mOnErrorListener.onError(STATE_ERROR_CODE);
            return false;
        }
        if (!mCurrentFilePath.isEmpty()) {
            LogUtil.i(TAG, "<discardRecord> mCurrentFilePath = " + mCurrentFilePath);
    ...
    public boolean playFile(String path) {
        LogUtil.i(TAG, "<playFile> path = " + path);
        if (null == path) {
            return false;
        }
    ...
    public boolean startPlayback() {
        LogUtil.i(TAG, "<startPlayback> in idle state, start play");
    ...
    public boolean pausePlay() {
        LogUtil.i(TAG, "<pausePlay> in play state, pause play");
        abandonAudioFocus();
    ...
    public boolean goonPlayback() {
        LogUtil.i(TAG, "<goonPlayback> in pause play state, goon play");
        boolean res = false;
        if (requestAudioFocus()) {
    ...
    public boolean stopPlay() {
        ...
        if ((STATE_PAUSE_PLAYING != mCurrentState) && (STATE_PLAYING != mCurrentState)) {
            LogUtil.i(TAG, "<stopPlay> not in play or pause play state, can't stop play");
            mOnErrorListener.onError(STATE_ERROR_CODE);
            return false;
        }
        abandonAudioFocus();
    ...
    public int getCurrentProgressInSecond() {
        ...
        LogUtil.i(TAG, "<getCurrentProgressInSecond> progress = " + progress);
        return progress;
    }
    ...
    public long getCurrentProgressInMillSecond() {
        LogUtil.i(TAG, "<getCurrentProgressInMillSecond> called");
        if (mCurrentState == STATE_PAUSE_PLAYING || mCurrentState == STATE_PLAYING) {
            int progress = mPlayer.getCurrentProgress();
            LogUtil.i(TAG, "<getCurrentProgressInMillSecond> progress = " + progress);
            return progress;
    ...
    public boolean isStorageMounted() {
        return Util.isStorageMounted(mStorageManager);
    }
    ...
    private Uri addToMediaDB(File file) {
        LogUtil.i(TAG, "<addToMediaDB> begin");
        if (null == file) {
            LogUtil.i(TAG, "<addToMediaDB> file is null, return null");
            return null;
        }
        ...
        cv.put(MediaStore.Audio.Media.TITLE, title);
        cv.put(MediaStore.Audio.Media.DATE_ADDED, (int) (current / FACTOR_FOR_SECOND_AND_MINUTE));
        LogUtil.v(TAG, "<addToMediaDB> File type is " + mCurrentRecordParams.mMimeType);
        ...
        cv.put(ALBUM_ARTIST, ALBUM_RECORDER);
        LogUtil.d(TAG, "<addToMediaDB> Reocrding time output to database is :DURATION= "
        ...
        try {
            ...
            LogUtil.e(TAG, "<addToMediaDB> Save in DB failed: " + e.getMessage());
        }
        if (null == result) {
            mOnErrorListener.onError(ErrorHandle.ERROR_SAVE_FILE_FAILED);
        } else {
            LogUtil.i(TAG, "<addToMediaDB> Save susceeded in DB");
    ...
    private Uri createPlaylist(Resources res, ContentResolver resolver) {
        ...
        try {
            ...
            LogUtil.e(TAG, "<createPlaylist> insert in DB failed: " + e.getMessage());
            uri = null;
        }
        /** @} */
     ...
     *
     * @param resolver   ContentResolver
     * @param audioId    the audio id that will be added to play list
     * @param playlistId the play list id which will add to
     */
    private void addToPlaylist(ContentResolver resolver, int audioId, long playlistId) {
        ...
        if (null == cur) {
            LogUtil.e(TAG, "<addToPlaylist> cursor is null");
            return;
        }
        cur.moveToFirst();
        ...
        try {
            ...
            LogUtil.e(TAG, "<addToPlaylist> insert in DB failed: " + e.getMessage());
            result = null;
        }
        if (null == result) {
            mOnErrorListener.onError(ErrorHandle.ERROR_SAVE_FILE_FAILED);
    ...
    private void registerBroadcastReceivcer() {
        if (null == mStorageBroastReceiver) {
            ...
            LogUtil.i(TAG, "<registerExternalStorageListener> register mStorageBroastReceiver");
        }
        ...
        if (null == mOtherBroastReceiver) {
            ...
            LogUtil.i(TAG, "<registerExternalStorageListener> register mOtherBroastReceiver");
        }
    }
    ...
    private String deleteRecordingFileTmpSuffix() {
        LogUtil.i(TAG, "<deleteRecordingFileTmpSuffix>");
        if (!mCurrentFilePath.endsWith(Recorder.SAMPLE_SUFFIX)) {
            return null;
        }
        File file = new File(mCurrentFilePath);
        if (!file.exists()) {
            LogUtil.i(TAG, "<deleteRecordingFileTmpSuffix> file is not exist.");
            return null;
        }
    ...
    private void setState(int stateCode) {
        ...
        if (mOnStateChangedListener != null) {
            ...
            LogUtil.i(TAG, "<setState> mOnStateChangedListener = null, mCurrentState = "
                    + mCurrentState);
        }
    }
    ...
    private void showNotification(Context context) {
        if (!mShowNotifiaction) {
            hideNotifiaction();
            LogUtil.i(TAG, "<showNotification> mShowNotifiaction == false, return");
            return;
        }
        if (mCurrentState == STATE_IDLE) {
            LogUtil.i(TAG, "<showNotification> not show in STATE_IDLE, return");
            return;
        }
        LogUtil.i(TAG, "<showNotificatoin> create mNotificationView");
        mNotificationView = new RemoteViews(getPackageName(), R.layout.layout_notification);
        ...
        LogUtil.i(TAG, "<showNotificatoin> startForeground");
        startForeground(START_NOTIFICATION_ID, notification);
        mRunForeground = true;
    }

    private void hideNotifiaction() {
        LogUtil.i(TAG, "<hideNotifiaction>");
        stopForeground(true);
        mRunForeground = false;
    }

    private void setNotification() {
        LogUtil.i(TAG, "<setNotification>");
    ...
    private class RecordingFileObserver extends FileObserver {
        ...
        public void onEvent(int event, String path) {
            LogUtil.i(TAG, "<RecordingFileObserver.onEvent> event = " + event);
            if (!mHasSendMessage) {
                if ((FileObserver.DELETE_SELF == event) || (FileObserver.ATTRIB == event)
                        || (FileObserver.MOVE_SELF == event)) {
                    LogUtil.i(TAG, "<RecordingFileObserver.onEvent> " + mWatchingPath
        ...
        if (null != mCurrentFilePath) {
            ...
            if (null == mFileObserverHandler) {
                mFileObserverHandler = new Handler(getMainLooper()) {
                    public void handleMessage(android.os.Message msg) {
                        LogUtil.i(TAG, "<mFileObserverHandler handleMessage> reset()");
                        resetAsync();
                    }
            ...
            LogUtil.i(TAG, "<setCurrentFilePath> start watching file <" + mCurrentFilePath + ">");
    ...
    private boolean requestAudioFocus() {
        if (!mGetFocus) {
            ...
            if (result != AudioManager.AUDIOFOCUS_REQUEST_GRANTED) {
                LogUtil.i(TAG, "<requestAudioFocus> request audio focus fail");
                mGetFocus = false;
            } else {
                LogUtil.i(TAG, "<requestAudioFocus> request audio focus success");
                mGetFocus = true;
            }
        }
        return mGetFocus;
    ...
    /**
     * new private function for abandon audio focus, it will be called when.
    ...
    private void abandonAudioFocus() {
        if (mGetFocus && (null != mAudioManager) && (null != mFocusChangeListener)) {
            if (AudioManager.AUDIOFOCUS_REQUEST_GRANTED == mAudioManager
                ...
                LogUtil.i(TAG, "<abandonAudioFocus()> abandon audio focus success");
                mGetFocus = false;
            } else {
                LogUtil.e(TAG, "<abandonAudioFocus()> abandon audio focus failed");
                mGetFocus = true;
            }
        }
    }
    ...
    /**
     * check if the storage Uri is current operation path.
     *
     * @param storageUri same as file:///storage/sdcard0
    ...
    public boolean isStorageLower() {
        try {
            if (Util.isStorageMounted(mStorageManager)) {
                String storageDirectory =
                        Environment.getExternalStorageDirectory().getAbsolutePath();
                /**
                 * Modified for SD card hot plug-in/out. Should to check the
                ...
                try {
                    ...
                    LogUtil.d(TAG, "stat " + storageDirectory + " failed...");
                    return false;
                }
    ...
    /**
     * set current record format when user select an item in SelectDialogFragment.
     *
    ...
    public void storeRecordParamsSettings() {
        LogUtil.i(TAG, "<storeRecordParamsSettings> ");
        if (null == mPrefs) {
        ...
        ed.commit();
        LogUtil.i(TAG, "mSelectedFormat is:" + mSelectedFormat);
    }
    ...
    public void startRecordingAsync(
            RecordParamsSetting.RecordParams recordParams, int fileSizeLimit, OnUpdateButtonState callback) {
        LogUtil.i(TAG, "<startRecordingAsync>");
        if (mCurrentState == STATE_RECORDING) {
            LogUtil.d(TAG, "already in recording state, not disable button");
            return;
        }
        if (callback != null) {
    ...
    public void doPlayRecord(OnUpdateButtonState callback) {
        if (STATE_RECORDING == mCurrentState
            ...
            LogUtil.i(TAG, "<playCurrentFile> in record or pause record state, can't play");
            return;
    ...
    public void doStop(OnUpdateButtonState callback) {
        if ((SoundRecorderService.STATE_PAUSE_PLAYING == mCurrentState)
            ...
            LogUtil.i(TAG, "<onClickStopButton> mService.stopPlay()");
            if (callback != null) {
                callback.updateButtonState(false);
            }
            stopPlaybackAsync();
                ...
                || (SoundRecorderService.STATE_PAUSE_RECORDING == mCurrentState)) {
            LogUtil.i(TAG, "<onClickStopButton> mService.stopRecord()");
            if (callback != null) {
                callback.updateButtonState(false);
            }
            stopRecordingAsync();
    ...
    public void doSaveRecord(OnUpdateButtonState callback) {
        ...
        if ((SoundRecorderService.STATE_PAUSE_PLAYING == mCurrentState)
                || (SoundRecorderService.STATE_PLAYING == mCurrentState)) {
            LogUtil.i(TAG, "<onClickAcceptButton> mService.stopPlay() first");
            stopPlaybackAsync();
        }
        saveRecordAsync();
    }
    ...
    public void doDiscardRecord(OnUpdateButtonState callback) {
        ...
        if ((SoundRecorderService.STATE_PAUSE_PLAYING == mCurrentState)
                || (SoundRecorderService.STATE_PLAYING == mCurrentState)) {
            LogUtil.i(TAG, "<onClickDiscardButton> mService.stopPlay() first");
            stopPlaybackAsync();
        }
        discardRecordAsync();
    }
    ...
    public void pauseRecordingAsync() {
        LogUtil.i(TAG, "<pauseRecordingAsync>");
    ...
    public void stopRecordingAsync() {
        LogUtil.i(TAG, "<stopRecordingAsync>");
    ...
    public void startPlaybackAsync() {
        LogUtil.i(TAG, "<startPlaybackAsync>");
    ...
    public void pausePlaybackAsync() {
        LogUtil.i(TAG, "<pausePlaybackAsync>");
    ...
    public void goonPlaybackAsync() {
        LogUtil.i(TAG, "<goonPlaybackAsync>");
    ...
    public void stopPlaybackAsync() {
        LogUtil.i(TAG, "<stopPlaybackAsync>");
    ...
    public void discardRecordAsync() {
        LogUtil.i(TAG, "<discardRecordAsync>");
